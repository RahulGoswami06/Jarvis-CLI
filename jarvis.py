...
try:
    from facial_auth import (
        ...
        cancel_auth,
        get_auth_status,
        CV2_AVAILABLE as FACIAL_AVAILABLE,
    )
except ImportError:
    FACIAL_AVAILABLE = False
    load_known_faces = None
    register_new_face = None
    verify_face = None
    authentication_thread = None
    cancel_auth = None
...
pending_message = {"to": None, "message": None, "scheduled_time": None}
...
CONTACTS = {
    "beena": "+61485527434",
    "beena sis australia": "+61485527434",
    "me": "+919316282910",
    "kuldeep": "+917874359837",
    "papa": "+919824272432",
    "kavan": "+917069468799",
}
...
INTENT_PATTERNS = {
    ...
    "schedule_message": [
        r"schedule\s+(?:a\s+)?message",
        r"send\s+(?:message\s+)?at\s+(.+)",
        r"message\s+at\s+(.+)",
        r"send\s+(?:this\s+)?later",
    ],
    "memory_store": [
        r"remember\s+(?:that\s+)?(.+)",
        r"store\s+(.+)",
...
def parse_schedule_time(time_str):
    ...
    patterns = [
        ...
        (r"in\s+(\d+)\s*(minute|min|hour|hr|second|sec)s?", "delta"),
        (r"today\s+at\s+(\d{1,2}):(\d{2})\s*(am|pm)?", "time"),
        (r"tomorrow\s+at\s+(\d{1,2}):(\d{2})\s*(am|pm)?", "time"),
    ]
...
def send_whatsapp_message(to, message, scheduled_time=None):
    import subprocess
    import pyautogui
    import time
    ...
    lookup_key = to.lower().strip()
    if lookup_key in CONTACTS:
        phone = CONTACTS[lookup_key]
    else:
        phone_match = re.search(r"[\d\s\-()+]{10,}", to)
        if phone_match:
    ...
    if scheduled_time:
        ...
        return f"Message scheduled for {time_str}. To: {phone}, Message: {message}"
    ...
    try:
        ...
        return f"Sent to {lookup_key}: {message}"
    except Exception as e:
        return f"Error: {str(e)}. Try WhatsApp Web."
...
def main():
    ...
    if FACIAL_AVAILABLE:
        from facial_auth import load_known_faces, verify_face, known_names as fa_names
        try:
            if load_known_faces() and fa_names:
                jarvis_print("Scanning face for authentication...")
                ...
                for attempt in range(3):
                    try:
                        ...
                        if recognized:
                            ...
                            jarvis_print("Authentication successful!")
                            time.sleep(0.3)
                            speak(f"Welcome {name}, systems online.")
                            break
                        else:
                            print(f"  Attempt {attempt + 1}/3 - Face not recognized")
                            if attempt < 2:
                                time.sleep(3)
                    except Exception as e:
                ...
                if not success:
                    print("\n  ⚠ Authentication failed - Starting anyway")
            else:
                print("  [FACIAL] No faces registered. Skipping auth.")
        except Exception as e:
            print(f"  [FACIAL] Error: {e}. Skipping auth.")
...
[ MESSAGE COMMANDS ]
  send message   - Start WhatsApp message (asks recipient, then message)
  to <person>    - Set recipient after 'send message'
  message <text> - Set message content
  now send      - Send immediately
  send at 5pm   - Schedule for specific time

[ FACIAL RECOGNITION ]
  scan face       - Start facial recognition (3 attempts)
  register-face  - Register your face
  stop scan      - Cancel facial scan
...
""")
            ...
            elif user_input.lower().startswith("to ") and pending_message.get("message") is None:
                pending_message["to"] = user_input[3:].strip()
                jarvis_print(f"Message will be sent to: {pending_message['to']}")
                jarvis_print("What message would you like to send?")

            elif user_input.lower().startswith("message ") and pending_message.get("to"):
                pending_message["message"] = user_input[8:].strip()
                jarvis_print(f"Message: {pending_message['message']}")
                jarvis_print("When should I send it? (say 'now send' or 'send at 5:30 pm')")

            elif user_input.lower().startswith("send at ") and pending_message.get("to") and pending_message.get("message"):
                time_str = user_input[8:].strip()
                scheduled = parse_schedule_time(time_str)
                if scheduled:
                    pending_message["scheduled_time"] = scheduled
                    ...
                    jarvis_print("Could not understand the time. Try 'send now' or 'send at 5:30 pm'")

            elif user_input.lower().startswith("message ") or "send message" in user_input.lower():
                pending_message["to"] = None
                pending_message["message"] = None
                pending_message["scheduled_time"] = None
                jarvis_print("Who should I send the message to? (Enter phone number or name)")

            elif command in ["now send", "send now", "right now", "send right now", "go send", "send immediately"]:
                if pending_message.get("to") and pending_message.get("message"):
                    result = send_whatsapp_message(
                        pending_message["to"],
                        pending_message["message"],
                        None
                    ...
                    jarvis_print("No message to send. Use 'send message' first.")

            elif command in ["scan face", "face scan", "facial scan", "recognize face"]:
                if not FACIAL_AVAILABLE:
                    jarvis_print("Facial recognition not available. Install opencv-python face_recognition")
                else:
                    def on_auth_success(name):
                        jarvis_print(f"Face recognized! Welcome {name}!")
                        speak(f"Welcome {name}")
                    
                    authentication_thread(max_attempts=3, on_success=on_auth_success)
                    jarvis_print("Scanning face... (3 attempts)")

            elif command == "stop scan":
                if FACIAL_AVAILABLE:
                    cancel_auth()
                    jarvis_print("Face scan cancelled")

            elif command == "register-face" or command.startswith("register face "):
                if not FACIAL_AVAILABLE:
                    jarvis_print("Facial recognition not available.")
                else:
                    name = user_input.replace("register face", "").strip() if "register face" in command else None
                    if not name:
                        jarvis_print("Enter name to register:")
                        name = sys.stdin.readline().strip()
                    if name and register_new_face(name):
                        jarvis_print(f"Face registered: {name}")
                    else:
                        jarvis_print("Failed to register face")
